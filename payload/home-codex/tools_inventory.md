...
| pwntools | `python3 -c 'import pwnlib; print(pwnlib.__version__)'` | apt | OK | `4.15.0` |  |
...
| z3 | `python3 -c 'import z3; print(z3.get_version_string())'` | pip/apt | OK | `4.13.0` |  |
| sage | `sage --version || readlink -f "$(command -v sage)"` | micromamba/local | OK | `SageMath / command available` | Original Kali setup used a user-local micromamba env. |
| angr | `python3 -c 'import angr; print(angr.__version__)'` | pip | OK | `9.2.217` |  |
...
| yosys | `yosys -V` | oss-cad-suite | OK | `Yosys 0.65+57 (git sha1 9d0cdb855, clang++ 18.1.8 -fPIC -O3)` | PATH must include /opt/oss-cad-suite/bin. |
| iceunpack | `iceunpack 2>&1 \| head -1` | apt | OK | `Error: Unexpected end of file.` |  |
| icebox_vlog | `icebox_vlog -h 2>&1 \| head -1` | apt | OK | `` |  |
| bitwuzla | `bitwuzla --version` | oss-cad-suite | OK | `1.0-prerelease` | PATH must include /opt/oss-cad-suite/bin. |
...
| ffuf | `ffuf -V` | apt/go | OK | `ffuf version: 2.1.0-dev` | guard blocks broad scans unless scoped/approved |
